...
class CIPointWiseEnv(gym.Env):
    def __init__(self, cycle_logs: CICycleLog, conf: Config):
        ...
        self.observation_space = spaces.Box(
            low=0, high=1, shape=(1, self.testcase_vector_size)
        )  # ID, execution time and LastResults

    def get_point_data(self, test_case_index):
        temp_obs = self.cycle_logs.export_test_case(
    ...
    def _calculate_reward(self, test_case_prob):
        ...
        optimal_rank = self.optimal_order.index(
            self.cycle_logs.test_cases[self.current_index]
        )
