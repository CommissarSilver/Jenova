...
class CICycleLog:
        ...
        if option == "list_avg_exec_with_failed_history":
            ...
            for test_case in self.test_cases:
                test_cases_array[i] = self.export_test_case(
                    test_case, "list_avg_exec_with_failed_history", win_size=winsize
                )
                i = i + 1
    ...
    def get_optimal_order(self):
        optimal_order_by_verdict = copy.deepcopy(
            sorted(self.test_cases, key=lambda x: x["verdict"], reverse=True)
        )
        optimal_order = []
    ...
    def calc_APFD(self, ordered_test_cases_id):
        ...
        for i in range(0, len(ordered_test_cases)):
            if ordered_test_cases[i] < self.get_test_cases_count():
                sum_ranks = sum_ranks + self.test_cases[ordered_test_cases[i]][
                    "verdict"
                ] * (i + 1)
        N: float = self.get_test_cases_count()
        M: float = self.get_failed_test_cases_count()
    ...
    def calc_optimal_APFD(self):
        optimal_order = sorted(
            self.test_cases, key=lambda x: x["verdict"], reverse=True
        )
        sum_ranks = 0
        i = 1
        apfd: float = 0
